...
export const getNotConductedTestPaper = () => async (dispatch) => {
  try {
    ...
    let { data } = await httpService.get("/api/test/details/all", Token());
...
export const getConductedTestPaper = () => async (dispatch) => {
  try {
    dispatch({ type: test.TEST_LIST_REQUEST });
    let { data } = await httpService.get(
      "/api/test/conducted/details/all",
      Token()
    );
...
export const testBegin = (id, index, testPapers) => async (dispatch) => {
  try {
    await httpService.post("/api/test/begin", { id }, Token());
...
export const testEndByTeacher = (testPapers, id) => async (dispatch) => {
  try {
    const { data } = await httpService.post("/api/test/end", { id }, Token());
...
export const getTestDetails = async (id) => {
  try {
    const { data } = await httpService.get(`/api/test/get/${id}`, Token());
    return data;
  } catch (ex) {
    errorHandler(ex);
  }
